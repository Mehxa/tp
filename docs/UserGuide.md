...
* In this document, there are **symbol buttons** following a command's format, which represent the parameters of the corresponding command. The colours indicate whether the parameters are mandatory or optional.<br>
...
> This document also contains **attribute buttons**(e.g. [`NAME`](#input-validation-duplicate-handling-and-utilities), [`CERT_EXPIRY_DATE`](#input-validation-duplicate-handling-and-utilities)) which you can click on. This will bring you to their input validation rules summary table.

> This document also contains **command buttons**(e.g. [`add`](#adding-a-new-contact--add), [`find`](#searching-contacts-by-criteria--find)) which you can click on. If the `delete` command section mentions `list`, `list` command will be clickable. This will bring you to other command sections so you do not have to scroll manually to view the other commands' usage details.
...
* You may use a prefix more than once to expand the search (i.e. **OR** search) (see Example 1), the only exception is [`CERT_EXPIRY_DATE`](#input-validation-duplicate-handling-and-utilities).
  * Use repeated prefixes, not spaces, for **OR** (i.e. `find t/HR t/IT` and not `find t/HR IT`)
* You may also use multiple different arguments to narrow down the search (i.e. **AND** search) (see Example 3).
...
* Adds or deletes tags of the person at the specified [`INDEX`](#input-validation-duplicate-handling-and-utilities) of the displayed person list.
* **You must either specify the add `a/` prefix or the delete `d/` prefix. Both arguments cannot be specified at the same time.**
* If multiple tags are to be added or deleted, you should separate their names by spaces. (e.g. `a/TAG1 TAG2`). Note that the `tag` command is the only command with this property.
* There are 5 colour options for `COLOUR_OF_TAGS_TO_ADD`: `RED`, `YELLOW`, `GREEN`, `BLUE` (default), and `PURPLE`.
  * You can specify one colour to match 1-for-1 per tag in `a/`, separated by space. (i.e `a/TAG1 TAG2 c/RED GREEN`. Note that the specified number of tags and colours **must** be the same. See Example 4 for more details)
...
* Expiry dates are optional (**omit** `e/` for Certificates that have no expiry date).
...
* Overwriting a Certificate with the same [`CERT_NAME`](#input-validation-duplicate-handling-and-utilities) and [`CERT_EXPIRY_DATE`](#input-validation-duplicate-handling-and-utilities) is allowed.
...
> `undo` only works after a data-modifying command occurred in the **current** user session. Meaning that if you ran [`clear`](#clearing-all-entries--clear), then exited the app, then reopened the app to run `undo`, the new session will **not** track the previous session's [`clear`](#clearing-all-entries--clear) command, causing `undo` to fail in this new user session. 
...
* The full contact list will be sorted, however filtered entries remain filtered.
  * e.g. if [`find`](#searching-contacts-by-criteria-find) was called before `sort`, only the found contacts remain displayed.
...
> If you accidentally ran `clear`, you can run [`undo`](#restoring-the-contact-list-undo)  to restore the cleared entries, provided you are in the **same** user session.
</box>
...
|------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NAME             | 1. Cannot be empty<br>2. Only letters, whitespaces and forward slashes<br/>3. Letters immediately beside forward slash must be uppercase (e.g. `S/O`)<br>4. Non-english names should be romanised before input.<br>5. To protect against incorrect parsing, names should not contain any substring that matches a command prefix case-sensitively (e.g. `sal/`, `p/`, `e/`, `a/`, `n/`, etc.).                                                                                                                                                                         | case-*insensitive* comparison                                    | 1. Leading, trailing and internal whitespaces for `/` will be trimmed (e.g.   `S   /  O` will be trimmed to `S/O`). <br/> 2. Internal whitespaces between words will be trimmed to 1.                                                                      |
...
| EMAIL            | 1. Can be empty<br>2.  Emails should be of the format `local-part@domain`, where `local-part` should:<br/>a .contain only alphanumeric characters and `+_.-`<br/>b. not start or end with `+_.-`<br/> c. not contain consecutive `+_.-`<br/>3. and `domain` is made of at least 2 domain labels where each should:<br>a. be separated by `.`<br/>b. contain only alphanumeric characters and hyphens<br/>c. not contain consecutive hyphens<br/>d. start and end only with alphanumeric characters<br/>e. be at least 2 characters long for the last domain label<br/> | case-*sensitive* comparison                                      | Leading, trailing and internal whitespaces will be trimmed.                                                                                                                                                                                                |
